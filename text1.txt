...
-- ==================== SERVICES & VARIABLES (FIXED FOR ANDROID) ====================
...
-- [BYPASS DELTA 1] Ambil CoreGui dengan aman pakai gethui()
local CoreGui
if gethui then
    CoreGui = gethui()
else
    local success, result = pcall(function() return game:GetService("CoreGui") end)
    CoreGui = success and result or LocalPlayer:WaitForChild("PlayerGui")
end

-- [BYPASS DELTA 2] Ambil VirtualInputManager pake pcall biar ga crash
...
pcall(function()
    TeleportService.TeleportInitFailed:Connect(function() isHopping = false end)
end)
...
-- ==================== SYSTEM DRAG UNTUK MOBILE (ANDROID FIX) ====================
...
-- ==================== THE GOD MODE QTE LOOP (ANDROID STABLE) ====================
...
local function ExecuteClickUI()
    ...
    pcall(function()
        ...
        if actionBtn then
            ...
            if VirtualInputManager and not isMobile then
                pcall(function()
    ...
    while task.wait(1) do
        ...
        if autoFarmStatus == "IDLE" or autoFarmStatus == "WAITING_IN_LOBBY" then
            ...
            elseif teamName:find("survivor") or teamName:find("player") then
                ...
                elseif autoFarmStatus == "WAITING_IN_LOBBY" then
                    if tick() - autoFarmWaitTime >= 150 then
        ...
        elseif autoFarmStatus == "WAITING_SPECTATOR" then
            if teamName:find("spectator") or teamName:find("lobby") or teamName:find("menu") then
                ...
                autoFarmWaitTime = tick()
            elseif tick() - autoFarmWaitTime >= 20 then 
